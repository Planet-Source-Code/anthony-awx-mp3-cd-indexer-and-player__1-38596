...
Public Sub getMP3Info(ByVal lpMP3File As String, ByRef lpMP3Info As Mp3Info)
  ...
  ff = FreeFile
  Open lpMP3File For Binary As ff
    Get ff, 1, Buf
  Close ff
...
Public Function GetTextFromFile(ByVal fn$) As String
...
Public Sub PlayMP3(Filename$, Summary$, cdVolume$)

    Filename = Replace(Filename, "\\", "\")
    
    Do Until CDAvailable(Filename) = True
    
        ui = MsgBox("Please insert the " _
           & cdVolume & " CD into " _
             & Trim$(Config.iDefaultPlayerDrive), _
             vbInformation + vbOKCancel, _
             "Insert CD")
        If ui = vbCancel Then Exit Sub
    
    Loop
        
    Dim MP3Player As New frmPlayer
    
    With MP3Player
        .Text1.Caption = Replace(Filename$, "\\", "\")
        .lblSongTitle = Replace(Right$(Filename, Len(Filename) - InStrRev(Filename, "\")), ".mp3", "")
        .Text2.Text = Summary$
        .Show
        .CmdPlay_Click
    End With
...
Public Sub doApplyTranslucency(frm As Form)

'// APPLIES TRANSLUCENY AS PER PROFILE SETTING          //
'// TO NON-MDI CHILD FORMS                              //
'//                                                     //
'// IF ACCESSED OUTSIDE OF PROFILE, NO TRANSLUCENCY IS  //
'// APPLIED                                             //

    Dim translucency As Byte
    Dummy = TranslucentForm(frm, 200)
    
End Sub

Public Function CDAvailable(ByVal Filename$) As Boolean
